        ...
        for address, data in analysis_data['results'].items():
            if 'error' in data:
                ...
                csv_data.append({
                    ...
                    'Total_Volume_Native': round(data['total_volume_native'], 6),
...
def main():
    ...
    print("📊 Base Network Token Analysis")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    def calculate_our_24h_volume(self, contract_address: str, transfers: List[Dict]) -> float:
        """Calculate our own 24h volume for comparison"""
        token = self.tokens[contract_address.lower()]
        ...
        return volume_24h, transfers_24h
    ...
    def validate_with_dexscreener(self, analysis_data: Dict) -> Dict:
        ...
        for address, data in analysis_data['results'].items():
            ...
            # We need to recalculate from transfers for 24h period
            transfers = self.api_client.get_token_transfers(address)
            zero_address = "0x0000000000000000000000000000000000000000"
            trading_transfers = [
                tx for tx in transfers 
                if tx.get('from', '').lower() != zero_address.lower() 
            ...
            our_volume_24h_native, our_transfers_24h = self.calculate_our_24h_volume(address, trading_transfers)
